...
def sync_to_web():
    ...
    files = ["people.json", "graph.json", "black_book.json", "flight_manifests.json", "court_does.json"]
    for fname in files:
        s = src_dir / fname
        d = dest_dir / fname
        if s.exists():
...
def main():
    ...
    print("[Step 1/8] Ingesting Wikipedia 'Named in Files' list...")
    run_scraper()
    print("\n---------------------------------------------------------")

    print("[Step 2/8] Ingesting Wikipedia 'Connections & Orgs' article...")
    run_connections_scraper()
    print("\n---------------------------------------------------------")

    print("[Step 3/8] Ingesting Court Dockets (Does) & Flight Logs...")
    run_court_and_flight_records()
    print("\n---------------------------------------------------------")

    print("[Step 4/8] Unifying & enriching entities, properties, and relations...")
    run_enricher()
    print("\n---------------------------------------------------------")

    print("[Step 5/8] Generating multi-source graph network dataset...")
    run_export_graph()
    print("\n---------------------------------------------------------")

    print("[Step 6/8] Processing Epstein's Address Book (The 'Little Black Book')...")
    run_black_book_loader()
    print("\n---------------------------------------------------------")

    print("[Step 7/8] Processing Flight Logs & Aviation Manifests...")
    run_flight_logs_loader()
    print("\n---------------------------------------------------------")

    print("[Step 8/8] Generating Court Does (1–187) Judicial Index...")
    ...
    print("  - people.json          (186 structured dossiers)")
    print("  - graph.json           (210 nodes, 564 edges)")
